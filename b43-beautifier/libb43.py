...
class B43SymbolicLabel:
	"""This class converts label names into symbolic labels."""

	def __init__(self, header_file):
		"""The passed header_file parameter is a file path to the
		file containing the label definitions."""
		try:
			defs = open(header_file).readlines()
		except IOError as e:
			print("B43SymbolicLabel: Could not read %s: %s" % (e.filename, e.strerror))
			B43Exception
		# Parse the definitions
		self.label_names = { }
		r = re.compile(r"^(L[0-9]+)_?\w*$")
		for line in defs:
			m = r.match(line)
			if not m:
				continue # unknown line
			longname = line.rstrip("\n")
			shortname = m.group(1)
			self.label_names[shortname] = longname

	def get(self, label):
		"""Get the symbolic name for an label. The label parameter
		must be a string like "LXXX", where X is a number."""
		try:
...
class B43SymbolicReg:
	"""This class converts numeric scratch pad register names into symbolic register names."""

	def __init__(self, header_file):
		"""The passed header_file parameter is a file path to the
		assembly file containing the symbolic scratch pad register definitions."""
		try:
			defs = open(header_file).readlines()
		except IOError as e:
			print("B43SymbolicReg: Could not read %s: %s" % (e.filename, e.strerror))
			B43Exception
		# Parse the definitions
		self.reg_names = { }
		...
		for line in defs:
			...
			name = m.group(1)
			offset = m.group(2)
			self.reg_names[offset] = name

	def get(self, reg):
		"""Get the symbolic name for an scratch pad register. The reg parameter
		must be a string like "rXX", where X is a number between 0 and 63."""
		try:
...
class B43AsmParser:
	...
	def __parse_code(self, asm_code):
		...
		calls = re.compile(r"^\s*(calls)\s+([\w]+)\s*$")
		...
		for line in asm_code.splitlines():
			m = label.match(line)
			if m: # Label:
				l = B43AsmLabel(m.group(1), line)
				self.codelines.append(l)
				continue
			m = calls.match(line)
			if m: # Calls, one operand
...
class B43Beautifier(B43AsmParser):
	...
	def __init__(self, asm_code, headers_dir):
		...
		self.symSpr = B43SymbolicSpr(headers_dir + "/ihr.inc")
		self.symShm = B43SymbolicShm(headers_dir + "/shm.inc")
		self.symCond = B43SymbolicCondition(headers_dir + "/cond.inc")
		self.symReg = B43SymbolicReg(headers_dir + "/scr.inc")
		self.symLabel = B43SymbolicLabel(headers_dir + "/labels.inc")
		self.preamble = "#include \"%s/ihr.inc\"\n" % headers_dir
	...
	def __process_code(self):
		...
		reg_re = re.compile(r"^r[0-9]{1,2}$")
		label_re = re.compile(r"^L[0-9]+$")
		code = self.codelines
		for line in code:
			#tst = re.compile(r"\s*calls.*$")
			#m = tst.match(line.getLine())
			#if m:
			#	print "calls found, is inst?: " + str(line.isInstruction())
			if line.isLabel():
				label = line.getLine()
				slabel = self.symLabel.get(line.getLabel())
				line.setLine(slabel + ":")
				continue
			...
			if opcode == "jext" and int(operands[0], 16) == 0x7F:
				...
				line.setOpcode("jmp")
				line.clearOperands()
				# Transform label operands
				m = label_re.match(label)
				if m:
					label = self.symLabel.get(label)
			...
			if opcode == "jext" or opcode == "jnext":
				...
				# Transform label operands
				m = label_re.match(operands[1])
				if m:
					operands[1] = self.symLabel.get(operands[1])
				continue
			# Transform calls labels
			if opcode == "calls":
				# Transform label operands
				m = label_re.match(operands[0])
				if m:
					operands[0] = self.symLabel.get(operands[0])
				continue
			...
			for i in range(0, len(operands)):
				...
				if m:
					...
					continue
				# Transform REG operands
				m = reg_re.match(o)
				if m:
					operands[i] = self.symReg.get(o)
					continue
				# Transform label operands
				m = label_re.match(o)
				if m:
					operands[i] = self.symLabel.get(o)
					continue
